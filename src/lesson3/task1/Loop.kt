...
fun collatzSteps(x: Int): Int {
    ...
    var resX = x
    while (resX != 1) {
        if (resX % 2 == 0) resX /= 2 else resX = 3 * resX + 1
        count++
    }
    return count
}
...
fun hasDifferentDigits(n: Int): Boolean {
    val lastDigit = n % 10
    return digitCountInNumber(n, lastDigit) != digitNumber(n)
}
...
fun squareSequenceDigit(n: Int): Int {
    ...
    while (number > 0) {
        val curSqr = sqr(i)
        val curSqrDigit = digitNumber(curSqr)
        number -= curSqrDigit
        i++
    }
    return ((sqr(i - 1) * 10.0.pow(number)).toInt()) % 10
}
...
fun fibSequenceDigit(n: Int): Int {
    ...
    return ((fib(i - 1) * 10.0.pow(number)).toInt()) % 10
}
